...
export default class UseHealthpack extends NPCAction {
    ...
    public performAction(target: Battler): void {
        let healthpack = this.actor.inventory.find(item => item.constructor === Healthpack); //finds healthpack
        console.log("use health pach");
        if (healthpack != null && target.battlerActive){
            target.health += (healthpack as Healthpack).health;
